...
class ColorType extends baseType {
	...
	public function __construct($name, $type, $for_key, $pri_key, $length, $mandatory) {
		...
		$this->type="COLOR";  //beContent dependant
	}
	...
	public function connect($entity_name) {
		...
		$query= Parser::first_comma("create".$entity_name,", ")."{$this->name} VARCHAR(7) {$notNull}";
		return $query;
	}
 ...
 *
 */
class ColorTypeFactory implements baseTypeFactory
{
	function create($name, $type, $for_key, $pri_key, $length, $mandatory)
	{
		return new ColorType($name, $type, $for_key, $pri_key, $length, $mandatory);
	}
}
?>